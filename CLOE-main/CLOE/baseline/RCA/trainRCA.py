# from https://github.com/illidanlab/RCA/blob/main/trainRCA.py
...
class Solver_RCA:
    def __init__(
        ...
        data_path = "CLOE/datasets/" + data_name + ".npz"
        self.missing_ratio = missing_ratio
        self.model_save_path = "CLOE/baseline/RCA/trained_model/{}/{}/RCA/{}/".format(
            data_name, missing_ratio, seed
        )
        if oe == 0.0:
            self.result_path = "CLOE/results/{}/{}/RCA/".format(
                data_name, seed
            )
        else:
            self.result_path = "CLOE/results/RCA/{}/{}/RCA_{}/".format(
                data_name, seed, oe
            )
        ...
        if data_name == "9_census":
            test_size = 1- 3000/self.dataset.x[self.label==0].shape[0]
        elif data_name == "24_mnist":
            test_size = 1- 5000/self.dataset.x[self.label==0].shape[0]
        elif self.dataset.x[self.label==0].shape[0]<8000:
            test_size = 0.1
        else:
            test_size = 1- 8000/self.dataset.x[self.label==0].shape[0]
        X_train_valid, _= train_test_split(self.dataset.x[self.label==0], test_size=test_size, random_state=self.seed)
        training_data, _ = train_test_split(X_train_valid, test_size=0.2, random_state=self.seed)
        ...
        self.testing_loader = data.DataLoader(
            self.dataset, batch_size=self.n_test, shuffle=False
        )
        self.ae = None
    ...
    def test(self):
        ...
        pr = average_precision_score(gt, error)
        accuracy = accuracy_score(gt, pred)
        precision, recall, f_score, support = prf(gt, pred, average="binary")

        print(
            "Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f}, AUC : {:0.4f}, PR : {:0.4f}".format(
                accuracy, precision, recall, f_score, auc, pr
            )
        )
